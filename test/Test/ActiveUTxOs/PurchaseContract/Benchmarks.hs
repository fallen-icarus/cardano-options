...
module Test.ActiveUTxOs.PurchaseContract.Benchmarks where
...
-- all only have one possible `Terms`.
...
-- | Purchase ProposalUTxOs where all proposals are for different terms. They all only have one
-- `possibleTerms`.
...
-- | Purchase ProposalUTxOs where all proposals are for different terms. They all have three
-- `possibleTerms`. Which one is picked changes for each purchase.
...
benchTest3 number = do
  ...
  proposals <- txOutRefsAndDatumsAtAddress @ProposalDatum optionsAddress
  let desiredTerms = zip proposals $ cycle [0::Int,1,2]
