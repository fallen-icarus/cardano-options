...
module Test.ProposalUTxOs.CloseProposal.Regressions where
...
-------------------------------------------------
-- | Close a single valid Proposal UTxO. The Proposal UTxO has only one possible `Terms`.
...
-- | Close a single valid Proposal UTxO. The Proposal UTxO has three possible `Terms`.
...
-- | Close multiple valid Proposal UTxOs. The Proposal UTxOs are all for different conditions, and
-- they all only have one possible `Terms`.
...
-- | Close multiple valid Proposal UTxOs. The Proposal UTxOs are all for different conditions. The
-- proposals have different numbers of `possibleTerms`.
...
-- | Close multiple valid Proposal UTxOs that are stored at different writer addresses.
...
regressionTest6 = do
  ...
  proposalUTxOs <- concat <$>
    mapM (txOutRefsAndDatumsAtAddress @ProposalDatum) [optionsAddress1,optionsAddress2]
...
-- | Close a single invalid Proposal UTxO.
...
-- TestTree
-------------------------------------------------
-- | A `TestTree` containing all regression scenarios for closing Proposal UTxOs.
tests :: [TestTree]
tests =
