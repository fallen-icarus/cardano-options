...
import CardanoOptions hiding (posixTimeToSlot)
...
data References = References
  { proposalBeaconsRef :: TxOutRef
  , activeBeaconsRef :: TxOutRef
  , optionsRef :: TxOutRef
  , proxyRef :: TxOutRef
  , addressObserverRef :: TxOutRef
  } deriving (Show)
...
initializeReferenceScripts = do
    ...
    emptyTxParams
      { outputs =
          [ Output
              ...
              , outputReferenceScript = toReferenceScript $ Just proposalBeaconScript
              }
          ]
      , certificates =
          [ Certificate
              { certificateCredential = PV2.ScriptCredential proposalBeaconScriptHash
              , certificateWitness = 
                  StakeWithPlutusScript 
                    (toVersioned $ toLedgerScript proposalBeaconScript) 
                    (toRedeemer RegisterProposalScript)
    ...
    emptyTxParams
      { outputs =
          [ Output
              ...
              , outputDatum = PV2.NoOutputDatum
              , outputReferenceScript = toReferenceScript $ Just optionsScript
              }
          ]
      }
  ...
  void $ transact (Mock.mockWalletAddress w1) [refScriptAddress] [Mock.paymentPrivateKey w1] $
    emptyTxParams
      { outputs =
          [ Output
              { outputAddress = refScriptAddress
              , outputValue = LV.lovelaceToValue 3_000_000
              , outputDatum = PV2.NoOutputDatum
  ...
  void $ transact (Mock.mockWalletAddress w1) [refScriptAddress] [Mock.paymentPrivateKey w1] $
    emptyTxParams
      ...
      , certificates =
          [ Certificate
              { certificateCredential = PV2.ScriptCredential addressObserverScriptHash
              , certificateWitness = 
                  StakeWithPlutusScript 
                    (toVersioned $ toLedgerScript addressObserverScript) 
  ...
  References 
    <$> txOutRefWithReferenceScript (scriptHash proposalBeaconScript)
    <*> txOutRefWithReferenceScript (scriptHash activeBeaconScript)
    <*> txOutRefWithReferenceScript (scriptHash optionsScript)
    <*> txOutRefWithReferenceScript (scriptHash proxyScript)
    <*> txOutRefWithReferenceScript (scriptHash addressObserverScript)
  ...
  where
    ...
    prettyBalances ys =
      let toName sh
            ...
            | sh == scriptHash optionsScript = "OptionsAddress"
